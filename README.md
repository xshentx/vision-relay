...
首次启动后，在桌面窗口中配置文本模型和视觉模型即可使用本地 API。文本供应商列表中的“模型测试”可直接使用该供应商已配置的模型和 API Key 发送测试提示词，并显示响应内容、HTTP 状态及耗时；测试过程不会切换当前供应商或修改客户端路由。供应商编辑弹窗中的眼睛按钮可临时显示或隐藏完整 API Key。
...
如果没有设置 `CODEX_HOME`，则使用 `%USERPROFILE%\.codex`。只有调用客户端配置 API 时明确传入 `work_dir`，才会额外写入该项目的 `.codex\config.toml` 和 `.codex\vision-relay-model.json`，避免把 Vision Relay 自身的启动目录误当成项目目录。项目配置只包含 Codex 允许的模型、模型目录和沙箱设置；`model_provider`、`model_providers.*` 及认证设置始终保留在用户级配置中，避免 Codex 忽略项目配置并显示警告。
...
Windows 桌面版默认会在启动后访问 GitHub Releases 自动检查新版本，可在左侧“更新”页面关闭自动检测，也可以随时手动检查。发现新版本后，点击“下载更新并重启”，程序会：
...
```powershell
.\tools\build-windows.ps1 -Version v2.0.0
```
