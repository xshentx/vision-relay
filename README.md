...
- 一键配置不再写入本地 API Key 或固定占位令牌，并会清理旧版本写入的 Codex 托管认证。
- 关闭本地 API 后，已配置客户端可直连当前文本供应商，自动写入上游地址、真实令牌和已添加模型。
- 直连模式新增协议兼容性检查：Codex 支持 OpenAI Responses，Claude Code 支持 Anthropic，配置不兼容时会明确提示。
...
本地 API 不需要访问令牌，外部客户端可以直接调用所有兼容入口。一键配置不会写入 API Key、Bearer Token 或其他固定占位值；如果第三方客户端的界面强制要求填写 API Key，这是该客户端自身的限制，Vision Relay 本地 API 不会校验该值。关闭本地 API 后，客户端改为直连当前文本供应商，并写入供应商 API 地址、上游令牌和真实模型名；模型列表仍只包含当前供应商配置中已经添加的模型，不会自动导入上游的全部模型。
...
- 关闭本地 API 后，一键配置和文本供应商切换会让已配置客户端直连当前文本供应商，并写入供应商 API 地址、上游令牌和已添加模型的真实模型名（不会自动导入上游全部模型）；视觉模型中转不可用，文本模型的图片能力按每个模型的“支持多模态”设置写入客户端。 直连时 Codex 仅支持使用 Responses 协议的 OpenAI 兼容供应商，Claude Code 仅支持 Anthropic 协议供应商；协议不兼容或当前供应商未添加模型时会停止写入并给出提示。
...
- **切换第三方时保留官方登录**：默认开启。本地 API 开启时无需认证，一键配置不会覆盖 `%CODEX_HOME%\auth.json`，并会清理旧版本写入的托管认证。关闭本地 API、让 Codex 直连供应商时，开启该选项会保留官方认证并在 provider 配置中写入真实供应商令牌；如果关闭，Vision Relay 会先把官方认证备份到 `%CODEX_HOME%\vision-relay-auth.json`，再把真实供应商令牌写入托管认证，重新开启或恢复官方模式时会还原备份。
...
```powershell
.\tools\build-windows.ps1 -Version v1.6.0
```
